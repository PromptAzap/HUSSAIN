...
SECTIONS_DIR   = BASE_DIR / 'أقسام الدروس'
DB_PATH        = BASE_DIR / 'lectures_db.sqlite'

# ══════════════════════════════════════════════════════════
# أداة مساعدة: التحقق من وجود lecture_id في القاعدة
# ══════════════════════════════════════════════════════════
...
def run_import():
    ...
    if not SECTIONS_DIR.exists():
        print(f"❌ لم يتم العثور على مجلد الأقسام: {SECTIONS_DIR}")
        return

    conn = sqlite3.connect(str(DB_PATH))
    valid_ids = get_valid_lecture_ids(conn)
    print(f"[DB] عدد الدروس الصالحة في القاعدة: {len(valid_ids)}")
    ...
    # المرور على كل مجلد سلسلة
    series_dirs = sorted([d for d in SECTIONS_DIR.iterdir() if d.is_dir()])
    total_files = sum(len(list(d.glob('*.json'))) for d in series_dirs)
    print(f"[DIR] مجلدات السلاسل: {len(series_dirs)} | اجمالي الملفات: {total_files}\n")

    for series_dir in series_dirs:
        json_files = sorted(series_dir.glob('*.json'))
        if not json_files:
            continue
        print(f"  [+] {series_dir.name} ({len(json_files)} ملف)")
